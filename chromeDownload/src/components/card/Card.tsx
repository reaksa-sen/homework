...
export const Card:React.FC<CardProps> =({ item, handleRemoveCart, handleUpdate }) => {
  ...
  return (
    <div className=" mb-8">
      <div className="w-[48%] mx-auto flex flex-col gap-3">
        <div
          className={`w-full flex justify-between border-2 rounded-xl shadow-md bg-gray-300 ${item.borderColor}`}
        >
          <div
          ...
          >
            {item.image && typeof item.image === "string" && (
              <img src={item.image} alt="Uploaded" className="w-32 h-auto object-cover" />
            )}
          </div>
          <div
          ...
          <div className="w-[12%] flex justify-end mt-3 mr-4">
            <div className="h-6 rounded-full hover:bg-gray-200">
              <IoTrashOutline  size={23} onClick={handleShowConfirm} />
            </div>
